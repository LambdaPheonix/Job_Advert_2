...
function update_records_tbl(){ // this function creates and executes and update sql that take the sum of all dailies to make the overall
...
function upload_insta($job_title,$date,$uname = 'parallel'){ // functions uploads the daily clicks to the db 
    ...
    if(mysqli_num_rows($query)>0 && mysqli_num_rows($query)<2){ // if only one record for the day update it
        mysqli_close($con);
        return update_entry($job_title,1,$date);

    } elseif (mysqli_num_rows($query)>1){ // if more than 1 record for the daily it makes it one record and plus 1
        $clicks = 0;
        while($row = mysqli_fetch_assoc($query)){ // counts clicks for same date
...
function create_insert_sql($job_title,$clicks,$date){ // creates insert SQL for tbl_daily_clicks 
    $sql = "INSERT INTO tbl_daily_clicks (`job_title`, `daily_clicks`, `date`) VALUES ('$job_title','$clicks','$date')";
    return $sql;
}

function create_update_sql($job_title,$clicks,$date){ // note only works for tbl_daily_clicks
    return " UPDATE tbl_daily_clicks SET `daily_clicks` = `daily_clicks`+'$clicks' WHERE `job_title` = '$job_title' AND `date`='$date'; ";
}


function insert_entry($job_title,$clicks,$date){ // creates new daily record in db and updates overall record  
    $con = create_connection();
...
function update_entry($job_title,$clicks,$date){ // updates daily record and overall record
    $con = create_connection();
